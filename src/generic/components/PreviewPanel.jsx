 ...
 * Customizer presets + user-saved palettes via window.customifyStarterSites.
...
function getPalettes() {
	if ( typeof window !== 'undefined' ) {
		const fromHost = window.customifyStarterSites?.palettes;
 ...
 * pairs via `window.customifyStarterSites.fonts`.
 */
function getFonts() {
	if ( typeof window !== 'undefined' ) {
		const fromHost = window.customifyStarterSites?.fonts;
		if ( Array.isArray( fromHost ) && fromHost.length > 0 ) {
			return fromHost;
		}
	}
...
// `window.customifyStarterSitesRegisterStyleBuilder` from a companion plugin) — no edits
// in this component required.
//
// Theme picked from `window.customifyStarterSites.currentTheme` (set by the
...
function buildPreviewCss( palette, font ) {
	const themeSlug = typeof window !== 'undefined' && window.customifyStarterSites
		? ( window.customifyStarterSites.currentStylesheet || window.customifyStarterSites.currentTheme )
		: null;
	...
	try {
		...
		console.warn( '[custstsi] style builder threw', e );
		return '';
	}
}

const STEPS = [
	{ key: 'style', label: __('Style', 'customify-starter-sites') },
	{ key: 'plugins', label: __('Plugins', 'customify-starter-sites') },
...
const PHASES = [
	...
	// fetched (see Customify_Adapter::preinstall_fonts). The server keeps
	// the job status on `fetching` while this runs but bumps the percent
	// into this 8–15 band, so the row lights up on its own.
	{ key: 'installing_fonts', from: 8, to: 15, label: __('Installing fonts', 'customify-starter-sites') },
	{ key: 'installing_plugins', from: 15, to: 30, label: __('Installing required plugins', 'customify-starter-sites') },
	{ key: 'extracting', from: 30, to: 45, label: __('Extracting uploads', 'customify-starter-sites') },
	{ key: 'importing_content', from: 45, to: 90, label: __('Importing content', 'customify-starter-sites') },
	{ key: 'applying_options', from: 90, to: 100, label: __('Applying theme options', 'customify-starter-sites') },
];
...
export function PreviewPanel({ template, onClose }) {
	...
	// `window.customifyStarterSites.fonts`, from
	...
	//
	// Contract (documented for the Studio side). The message `type` is
	// kept as the original `fdi-preview-*` identifier: the Studio preview
	// iframe (served by the Studio server) listens for exactly this type,
	// so it must NOT be rebranded or the iframe stops receiving the CSS.
	//   {
	...
	const sendStyleToIframe = useCallback(() => {
		...
		try {
			...
			// eslint-disable-next-line no-console
			console.log(
				'[custstsi] preview-style →',
				{
					stylesheet:
						(typeof window !== 'undefined' && window.customifyStarterSites
							? window.customifyStarterSites.currentStylesheet ||
							  window.customifyStarterSites.currentTheme
							: null),
					font: currentFont,
			...
			// eslint-disable-next-line no-console
			console.log('[custstsi] preview-style CSS:\n' + css);
			win.postMessage(
				{
					// Keep the original identifier — the Studio preview
					// iframe listens for `fdi-preview-style`.
					type: 'fdi-preview-style',
					css,
				},
				'*'
	...
	// This identifier is fixed by the Studio preview iframe and must
	// stay `fdi-preview-ready` — don't rebrand it.
	useEffect(() => {
		const handler = (event) => {
			if (
	...
	// the overlay.
	useEffect(() => {
		document.body.classList.add('custstsi-wizard-open');
		return () => {
			document.body.classList.remove('custstsi-wizard-open');
		};
	}, []);
	...
	const plugins = useMemo(() => {
		// The Studio list snapshot ships the full plugin manifest at
		// `template.plugins[]` (each { id, slug, name, ... }). Older
		// schemas nested it under `requirements.plugins[]`, so fall back
		// to that for compatibility. Show every plugin the template
		// declares — do not drop any beyond the ecosystem-tooling excludes.
		const cfg =
			typeof window !== 'undefined' ? window.customifyStarterSites : null;
		const requiredSlugs = Array.isArray(cfg?.requiredPluginSlugs)
			? cfg.requiredPluginSlugs
			: [BLOCKSIFY_SLUG];
		const installedStates =
			cfg?.installedPlugins && typeof cfg.installedPlugins === 'object'
				? cfg.installedPlugins
				: {};

		const raw = Array.isArray(template?.plugins)
			? template.plugins
			: Array.isArray(template?.requirements?.plugins)
				? template.requirements.plugins
				: [];
		const list = raw.filter(
			(p) => p?.slug && !EXCLUDED_PLUGIN_SLUGS.includes(p.slug)
		);
		const mapped = list.map((p) => {
			const state = installedStates[p.slug] || {};
			return {
				slug: p.slug,
				name: p.name || p.slug,
				// Server-declared required set (blocksify + Pro), with the
				// Studio flag honoured as a fallback for legacy schemas.
				required:
					requiredSlugs.includes(p.slug) || Boolean(p.required),
				installed: Boolean(state.installed || p.installed),
		...
		const blocksifyState = installedStates[BLOCKSIFY_SLUG] || {};
		const blocksify = existing
			...
			: {
				...
				name: BLOCKSIFY_NAME,
				required: true,
				installed: Boolean(blocksifyState.installed),
				active: Boolean(blocksifyState.active),
	...
	const isPluginChecked = (p) => {
		...
		// Required plugins are always installed + activated.
		if (p.required) return true;
		// Recommended (incl. installed-but-inactive) default to checked so
		// they get installed/activated in the import — unless the user
	...
	// Recommended plugins that still need action (not already active) are
	// the ones the bulk toggle + skip flags apply to.
	const allOptionalUnchecked = recommendedPlugins.length > 0
		&& recommendedPlugins.filter((p) => !p.active).every((p) => pluginsSkip.includes(p.slug));

	const bulkToggleOptional = () => {
		const optActionable = recommendedPlugins.filter((p) => !p.active);
		setPluginsSkip(allOptionalUnchecked ? [] : optActionable.map((p) => p.slug));
	};
	...
	const handleStart = (overrides = {}) => {
		...
		// A null font means "use the template's default font" — not "no
		// font". The designer's first curated pair is that default, so
		// fall back to it when the user didn't actively pick one. This is
		// also what makes the "Installing fonts" step run for every
		// template that ships a typography list.
		const pickedFont    = pick('font',            currentFont);
		const finalFont     = pickedFont || (fonts.length > 0 ? fonts[0] : null);
		setStartError(null);
		setStarting(true);
		jobs.create({
			...
			.catch((err) => {
				setStartError(err.message || __('Failed to start import job.', 'customify-starter-sites'));
			})
	...
	const handleClose = () => {
		if (importing && !isDone) {
			...
			const ok = window.confirm(
				__('Importing in the background. Are you sure you want to leave?', 'customify-starter-sites')
			);
			if (!ok) return;
	...
	const visiblePhases = useMemo(() => PHASES.filter((p) => {
		if (p.key === 'installing_fonts') {
			// Shown whenever the template ships a font list — the import
			// installs the picked pair, or the template's default (first)
			// pair when the user didn't choose one. Hidden only when there
			// are no fonts to install at all.
		...
		return true;
	}), [plugins, pluginsSkip, contentEnabled, optWidgets, optCustomizer, palette, typography, fonts]);
				...
				<aside className="custstsi-sidebar">
					<header className="custstsi-sidebar__header">
						<div className="custstsi-sidebar__title">
							{__('Setting up:', 'customify-starter-sites')}
							<strong id="custstsi-wizard-title">{title}</strong>
						</div>
						<div className="custstsi-sidebar__divider" />
					</header>

					<div className="custstsi-sidebar__body">
						{showSteps && (
							<>
								{step === 0 && (
									<StyleStep
								...
								{step === 1 && (
									<PluginsStep
										...
										bulkLabel={allOptionalUnchecked
											? __('Check all', 'customify-starter-sites')
											: __('Uncheck all', 'customify-starter-sites')
										}
								...
								{startError && (
									<div className="custstsi-error">{startError}</div>
								)}
							</>
						)}
						...
						{showDone && (
							<DoneScreen onClose={handleClose} home={window.customifyStarterSites?.home || '/'} />
						)}
					</div>
							...
							<div className="custstsi-step-actions">
								...
								>
									{isFirst
										...
										: __('← Back', 'customify-starter-sites')
									}
								</Button>
								<div className="custstsi-step-actions__spacer" />
								<Button variant="tertiary" onClick={skip}>
									{__('Skip', 'customify-starter-sites')}
								</Button>
								<Button
									variant="primary"
									onClick={next}
						...
						{showInstall && (
							<div className="custstsi-step-actions">
								...
								<Button variant="secondary" isDestructive onClick={handleClose}>
									{__('Cancel import', 'customify-starter-sites')}
								</Button>
							</div>
						)}
					</footer>
...
function StyleStep({ palettes, palette, setPalette, typography, setTypography, fonts }) {
	...
	useEffect(() => {
		...
		familyToWeights.forEach((weights, family) => {
			const id = 'custstsi-font-' + family.toLowerCase().replace(/[^a-z0-9]+/g, '-');
	...
	return (
		<section>
			<h3 className="custstsi-step__heading">
			...
			<div className="custstsi-style-section">
				<h4 className="custstsi-style-subheading">
					{__('Color palette', 'customify-starter-sites')}
				</h4>
				<div className="custstsi-tile-grid">
					{palettes.map((p) => (
						<button
							key={p.id}
							type="button"
							className={'custstsi-tile' + (palette === p.id ? ' is-selected' : '')}
							onClick={() => setPalette(p.id)}
						>
							<span className="custstsi-tile__swatches">
								{p.colors.map((c, i) => (
			...
			<div className="custstsi-style-section">
				<h4 className="custstsi-style-subheading">
					{__('Typography', 'customify-starter-sites')}
				</h4>
				<div className="custstsi-tile-grid">
					<button
						type="button"
						className={'custstsi-tile custstsi-tile--skip' + (typography === null ? ' is-selected' : '')}
						onClick={() => setTypography(null)}
					>
						<span className="custstsi-tile__skip-dash" />
						<span className="custstsi-tile__label">{__('Keep current', 'customify-starter-sites')}</span>
					</button>
					{fonts.map((f) => {
						...
						return (
							<button
								key={f.id}
								type="button"
								className={'custstsi-tile custstsi-tile--font' + (typography === f.id ? ' is-selected' : '')}
...
function PluginsStep({ required, recommended, isChecked, onToggle, bulkLabel, onBulkToggle, loadingDetail }) {
	...
	const renderCard = (p) => {
		...
		// Locked (non-toggleable) when the plugin is required or already
		// active — required plugins are always installed, and active ones
		// need no further action. Recommended-but-inactive stays toggleable.
		const disabled = p.required || p.active;

		// A required plugin that isn't installed yet is flagged "Required".
		// These (Pro) plugins can't be installed automatically — they're
		...
		// for visual consistency, but it shows UNCHECKED — it can't be
		// installed automatically, so there's nothing to tick. Everything
		// else reflects its real checked state.
		const checked = requiredNotInstalled ? false : isChecked(p);
		if (checked) classes.push('is-checked');

		// A non-wp.org source badge still sits inline on the head row.
		const sourceBadge = ( ! p.installed && p.source && p.source !== 'wordpress.org' )
			? <span className="custstsi-plugin__source">{p.source}</span>
			: null;
		...
		return (
			...
			>
				<div className="custstsi-plugin__head">
					<span className="custstsi-plugin__name">{p.name}</span>
					{sourceBadge}
					<span className="custstsi-plugin__check" aria-hidden="true" />
				</div>
				{p.desc && (
					/*
					 * Single-line truncation via .custstsi-plugin__desc CSS
					 ...
					 */
					<div className="custstsi-plugin__desc" title={p.desc}>{p.desc}</div>
				)}
				{requiredNote}
			</div>
		);
	...
	return (
		<section>
			...
			{loadingDetail ? (
				...
				<div className="custstsi-plugins">
					{required.length > 0 && (
						<div className="custstsi-plugins-section">
							<h4 className="custstsi-plugins-section__title">
								{__('Required', 'customify-starter-sites')}
							</h4>
							<div className="custstsi-plugins-grid">
					...
					{recommended.length > 0 && (
						<div className="custstsi-plugins-section">
							<h4 className="custstsi-plugins-section__title">
								{__('Recommended', 'customify-starter-sites')}
								<button
									type="button"
									className="custstsi-plugins-section__bulk"
					...
					{required.length === 0 && recommended.length === 0 && (
						<p className="custstsi-step__hint">
							{__('This template doesn’t require any plugins.', 'customify-starter-sites')}
						</p>
					)}
				</div>
			)}
...
function ContentStep({ contentEnabled, setContentEnabled, optWidgets, setOptWidgets, optCustomizer, setOptCustomizer, showWarning }) {
	const toggleCard = (checked, label, desc, onToggle) => (
		<button
			type="button"
			className={'custstsi-plugin' + (checked ? ' is-checked' : '')}
			onClick={onToggle}
			aria-pressed={checked}
		>
			<div className="custstsi-plugin__head">
				<span className="custstsi-plugin__name">{label}</span>
				<span className="custstsi-plugin__check" aria-hidden="true" />
			</div>
			<div className="custstsi-plugin__desc">{desc}</div>
		</button>
	);
	...
	return (
		<section>
			<h3 className="custstsi-step__heading">
				{__('Content & options', 'customify-starter-sites')}
			</h3>
			<p className="custstsi-step__lede">
				{__('Choose what to import: demo content, widgets and Customizer settings.', 'customify-starter-sites')}
			</p>

			<div className="custstsi-style-section">
				<h4 className="custstsi-style-subheading">
					{__('Demo content', 'customify-starter-sites')}
				</h4>
				<div className="custstsi-plugins-grid">
					{toggleCard(
						contentEnabled,
						__('Import demo content', 'customify-starter-sites'),
						__('Sample posts, pages, categories and media so the site matches the demo.', 'customify-starter-sites'),
						() => setContentEnabled(!contentEnabled)
					)}
				</div>
				{showWarning && (
					<div className="custstsi-warning">
						{__('⚠ Widgets & menus may reference pages that won’t exist if you skip demo content.', 'customify-starter-sites')}
					</div>
				)}
			</div>
			...
			<div className="custstsi-style-section">
				<h4 className="custstsi-style-subheading">
					{__('Theme options', 'customify-starter-sites')}
				</h4>
				<div className="custstsi-plugins-grid">
					{toggleCard(
						optWidgets,
						__('Widgets', 'customify-starter-sites'),
						__('Sidebar and footer widget areas from the demo.', 'customify-starter-sites'),
						() => setOptWidgets(!optWidgets)
					)}
					{toggleCard(
						optCustomizer,
						__('Customizer settings', 'customify-starter-sites'),
						__('Header, footer, blog layout, container width, social links… (theme mods)', 'customify-starter-sites'),
...
function InstallProgress({ status, percent, phases, message, error, warnings }) {
	...
	return (
		<section className="custstsi-install">
			<div className="custstsi-install__overall">
				<div className="custstsi-install__pct">{pct}%</div>
				<div className="custstsi-install__label">
					{status === 'failed' ? __('Failed', 'customify-starter-sites')
						: status === 'queued' ? __('Starting…', 'customify-starter-sites')
							: (message || __('Working…', 'customify-starter-sites'))}
				</div>
				<div className="custstsi-install__bar">
					<div className="custstsi-install__bar-fill" style={{ width: `${pct}%` }} />
				</div>
			</div>

			<ol className="custstsi-install__phases">
				{phases.map((p) => {
					let cls = 'is-pending';
					if (percent >= p.to) cls = 'is-done';
					else if (percent >= p.from) cls = (status === 'failed' ? 'is-failed' : 'is-running');
					return (
						<li key={p.key} className={`custstsi-install__phase ${cls}`}>
							<span className="custstsi-install__phase-icon" aria-hidden="true" />
							<span className="custstsi-install__phase-label">{p.label}</span>
						</li>
					);
				})}
			</ol>
			...
			{status === 'failed' && (
				<div className="custstsi-status is-error">
					✗ {error || __('Import failed.', 'customify-starter-sites')}
				</div>
			)}
			{status === 'cancelled' && (
				<div className="custstsi-status is-warning">
					{__('Import cancelled.', 'customify-starter-sites')}
				</div>
			)}

			{Array.isArray(warnings) && warnings.length > 0 && (
				<details className="custstsi-warnings">
					<summary>
						{__('Warnings', 'customify-starter-sites')} ({warnings.length})
					</summary>
					<ul>
...
function DoneScreen({ onClose, home }) {
	return (
		<section className="custstsi-done">
			<div className="custstsi-done__check">✓</div>
			<h3 className="custstsi-done__heading">
				{__('Your site is ready', 'customify-starter-sites')}
			</h3>
			<p className="custstsi-done__lede">
				{__('Demo content has been imported and styling applied.', 'customify-starter-sites')}
			</p>
			<div className="custstsi-done__actions">
				<Button variant="primary" href={home}>
					{__('View site →', 'customify-starter-sites')}
				</Button>
				<Button variant="secondary" href="post-new.php?post_type=page">
					{__('Edit home page', 'customify-starter-sites')}
				</Button>
				<Button variant="secondary" href="customize.php">
					{__('Open Customizer', 'customify-starter-sites')}
				</Button>
