...
$custstsi_customify_adapter = new Customify_Starter_Sites\Adapters\Customify_Adapter();
Customify_Starter_Sites\Adapter_Registry::register( $custstsi_customify_adapter );
$custstsi_customify_adapter->register_integration();
unset( $custstsi_customify_adapter );
...
$custstsi_options = new Customify_Starter_Sites\Settings\Options_Store();
$custstsi_client  = new Customify_Starter_Sites\Studio\Remote_Client( $custstsi_options );
( new Customify_Starter_Sites\Generic_Dashboard( $custstsi_options ) )->register();
( new Customify_Starter_Sites\REST\Studio_Proxy_Controller( $custstsi_client ) )->register();
unset( $custstsi_options, $custstsi_client );
...
$custstsi_jobs      = new Customify_Starter_Sites\Jobs\Job_Store();
$custstsi_job_client = new Customify_Starter_Sites\Studio\Remote_Client( new Customify_Starter_Sites\Settings\Options_Store() );
$custstsi_runner    = new Customify_Starter_Sites\Jobs\Importer_Runner(
	$custstsi_job_client,
	$custstsi_jobs
);
$custstsi_runner->register();
// Pass the Studio client to the controller too, so it can look up a
// template's required plugins and reject Pro-gated imports server-side.
( new Customify_Starter_Sites\REST\Job_Controller( $custstsi_jobs, $custstsi_runner, $custstsi_job_client ) )->register();
unset( $custstsi_jobs, $custstsi_runner, $custstsi_job_client );
...
add_filter( 'custstsi_active_adapter', function () {
	static $cached = null;
	if ( null === $cached ) {
		$cached = Customify_Starter_Sites\Theme_Detector::active_adapter();
	}
	return $cached;
}, 10, 0 );
...
//   1. wp-config constant   (CUSTOMIFY_STARTER_SITES_STUDIO_URL/KEY)
//   2. Saved wp_option      (custstsi_studio_url/key)
...
add_filter( 'custstsi_default_studio_url', function ( $url ) {
	// wp-config constant wins — never override when it is set.
	if ( defined( 'CUSTOMIFY_STARTER_SITES_STUDIO_URL' ) && '' !== trim( (string) CUSTOMIFY_STARTER_SITES_STUDIO_URL ) ) {
		return $url;
	}
	...
	$saved = (string) get_option( Customify_Starter_Sites\Settings\Options_Store::OPT_STUDIO_URL, '' );
	if ( '' !== trim( $saved ) ) {
		return $url;
	}
 ...
 */
add_filter(
	'plugin_action_links_' . plugin_basename( CUSTOMIFY_STARTER_SITES_PATH . 'customify-starter-sites.php' ),
