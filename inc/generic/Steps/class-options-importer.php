 ...
 * successful gated run, `custstsi_settings_applied=1` marker is
...
class Options_Importer {
	...
	public const OPTION_SETTINGS_APPLIED = 'custstsi_settings_applied';
	...
	private function denied_keys(): array {
		 ...
		 */
		$keys = (array) apply_filters( 'custstsi_denied_option_keys', self::DENIED_KEYS );
		...
		foreach ( $pairs as $name => $path ) {
			...
			if ( $actual !== $expected ) {
				$warnings[] = sprintf(
					...
					__( '%1$s checksum mismatch (expected %2$s, got %3$s).', 'customify-starter-sites' ),
					$name,
					$expected,
					$actual
				);
	...
	private function check_theme_requirement( array $parsed, array &$warnings ): void {
		...
		if ( get_stylesheet() !== $wanted ) {
			$warnings[] = sprintf(
				...
				__( 'Theme mismatch: source used "%1$s", active is "%2$s". Theme switch is out of scope; mods may not apply cleanly.', 'customify-starter-sites' ),
				$wanted,
				get_stylesheet()
			);
		}
	...
	private function apply_core_page( string $option, array $core, array $ref_map, array &$warnings ): void {
		...
		$warnings[] = sprintf(
			...
			__( 'core.%s unresolved — left untouched.', 'customify-starter-sites' ),
			$option
		);
	}
		...
		foreach ( $mods as $key => $value ) {
			...
			if ( in_array( strtolower( $resolved_key ), $denied, true ) ) {
				$warnings[] = sprintf(
					...
					__( 'Skipped denylisted theme_mod: %s', 'customify-starter-sites' ),
					$resolved_key
				);
				continue;
			}
		...
		foreach ( $locations as $location => $term_id ) {
			...
			if ( isset( $this->term_pairs[ $src ] ) ) {
				...
				$warnings[] = sprintf(
					...
					__( 'Menu location "%1$s" unresolved (source menu %2$d not imported) — left unassigned.', 'customify-starter-sites' ),
					(string) $location,
					$src
				);
			}
		...
		foreach ( $cust as $key => $value ) {
			...
			if ( in_array( strtolower( $key_str ), $denied, true ) ) {
				$warnings[] = sprintf(
					...
					__( 'Skipped denylisted customizer key: %s', 'customify-starter-sites' ),
					$key_str
				);
				continue;
			}
	 ...
	 *
	 * Defensive: anything prefixed `pmbd_` or `custstsi_` is
	...
	private function apply_plugin_options( array $parsed, array $ref_map, array &$warnings ): bool {
		...
		foreach ( $plugin_options as $key => $value ) {
			...
			if ( 0 === strpos( $option_name, 'pmbd_' ) || 0 === strpos( $option_name, 'custstsi_' ) ) {
				$warnings[] = sprintf(
					/* translators: %s: the option key that starts with an importer-reserved prefix */
					__( 'Refused to import importer-prefixed option: %s', 'customify-starter-sites' ),
					$option_name
				);
				continue;
			}
			if ( in_array( strtolower( $option_name ), $denied, true ) ) {
				$warnings[] = sprintf(
					...
					__( 'Skipped denylisted plugin_option: %s', 'customify-starter-sites' ),
					$option_name
				);
				continue;
			}
		...
		foreach ( $fonts as $font ) {
			...
			if ( $family_id <= 0 ) {
				$warnings[] = sprintf(
					...
					__( "Font family '%s' could not be saved.", 'customify-starter-sites' ),
					$slug
				);
				continue;
			}
	...
	private function activate_font_in_global_styles( array $family_payload, array $face_settings_list, array &$warnings ): bool {
		...
		if ( null === $post ) {
			$warnings[] = sprintf(
				/* translators: %s: the font family slug (e.g. inter) */
				__( "Font '%s' was imported but could not be activated — no user wp_global_styles post.", 'customify-starter-sites' ),
				$slug
			);
			return false;
		}
		...
		if ( is_wp_error( $updated ) || (int) $updated <= 0 ) {
			$warnings[] = sprintf(
				/* translators: %s: the font family slug (e.g. inter) */
				__( "Font '%s' activation write to wp_global_styles failed.", 'customify-starter-sites' ),
				$slug
			);
			return false;
		}
				...
				if ( $src_host === $site_host && is_string( $path ) && false !== strpos( $path, '/wp-content/uploads/' ) ) {
					...
					if ( false !== $rel && ! file_exists( trailingslashit( $basedir ) . $rel ) ) {
						$warnings[] = sprintf(
							...
							__( 'Font file missing on disk: %s', 'customify-starter-sites' ),
							$rel
						);
					}
				}
