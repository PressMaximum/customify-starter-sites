...
class Uploads_Extractor {
	...
	public function extract( string $zip_path, bool $overwrite_existing = false ): array {
		if ( '' === $zip_path || ! file_exists( $zip_path ) ) {
			throw new \RuntimeException( esc_html( 'uploads.zip not found: ' . $zip_path  ));
		}
		if ( ! class_exists( 'ZipArchive' ) ) {
			throw new \RuntimeException( esc_html( 'PHP ZipArchive extension required.'  ));
		}

		$upload  = wp_get_upload_dir();
		$basedir = (string) ( $upload['basedir'] ?? '' );
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- Streaming a ZIP entry to disk requires a real file handle; WP_Filesystem has no streaming API, so writability is checked directly against the same path used for streaming.
		if ( '' === $basedir || ! is_writable( $basedir ) ) {
			throw new \RuntimeException( esc_html( 'wp-content/uploads is not writable.'  ));
		}
		$basedir_norm = wp_normalize_path( trailingslashit( $basedir ) );

		$zip = new \ZipArchive();
		if ( true !== $zip->open( $zip_path ) ) {
			throw new \RuntimeException( esc_html( 'Could not open uploads.zip.'  ));
		}
		...
		try {
			...
			for ( $i = 0; $i < $num; $i++ ) {
				...
				if ( $total > self::MAX_TOTAL_BYTES ) {
					throw new \RuntimeException( esc_html( 'uploads.zip exceeds 1 GB uncompressed cap.'  ));
				}
				...
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- Streaming a ZIP entry to disk requires a real file handle; WP_Filesystem has no streaming API.
				$out = @fopen( $target, 'wb' );
				if ( ! is_resource( $out ) ) {
					// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- Streaming a ZIP entry to disk requires a real file handle; WP_Filesystem has no streaming API.
					fclose( $stream );
