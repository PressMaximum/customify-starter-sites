...
namespace Customify_Starter_Sites\Adapters;
...
use Customify_Starter_Sites\Generic_Dashboard;
use Customify_Starter_Sites\Adapters\Customify\Font_Installer;
...
class Customify_Adapter extends Theme_Adapter {
	...
	/**
	 * Result of the early (post-fetch) font pre-install, consumed when the
	 * typography theme_mods are written in `applying_options`. Shape:
	 *   [ 'pair' => array, 'heading_id' => int|null, 'body_id' => int|null ]
	 * Null until preinstall_fonts() runs (or when there's no font style).
	 * Held on the instance — the runner keeps one adapter alive per job.
	 *
	 * @var array|null
	 */
	private ?array $preinstalled_fonts = null;
	 ...
	 * `CUSTOMIFY_STARTER_SITES_STUDIO_KEY` in wp-config (the bootstrap filter
	...
	public function register_integration(): void {
		...
		// The Customify dashboard hides its Starter Templates tab + submenu
		// unless `customify_use_starter_templates` is true. This importer
		// being active IS what makes the feature available, so turn the tab
		// on — otherwise the theme never registers the submenu or renders
		// the embed slot, and the plugin has no visible surface.
		add_filter( 'customify_use_starter_templates', '__return_true' );
		...
		add_filter( 'custstsi_show_generic_menu', '__return_false' );
	...
	public function inject_boot_metadata( array $boot ): array {
		$boot[ $this->embed_boot_key() ] = [
			'active'        => true,
			'version'       => defined( 'CUSTOMIFY_STARTER_SITES_VERSION' ) ? CUSTOMIFY_STARTER_SITES_VERSION : '1.3.0',
	...
	public function boot_payload(): array {
		...
		// Pro-plugin gating. Templates whose `plugins[]` reference a Pro
		// slug show a "Pro" badge, and can only be imported once the Pro
		// plugin is installed on the site (it may be inactive — the
		// importer activates it during the run). The wizard reads these
		// two lists to decide the badge + whether to disable Import.
		$out['proPlugins']          = self::pro_plugin_slugs();
		$out['installedProPlugins'] = self::installed_pro_plugin_slugs();

		// Install/active state for every plugin, so the wizard's Plugins
		// step can label already-present plugins and skip installing them.
		$out['installedPlugins'] = self::installed_plugin_states();

		// Slugs the wizard treats as "Required" (vs "Recommended"). The
		// block library and the Pro plugins are required to render a
	...
	/**
	 * Plugin slugs the wizard shows under "Required" rather than
	 * "Recommended". Filterable so the set can change without a release.
	 *
	 * @return string[]
	 */
	public static function required_plugin_slugs(): array {
		$slugs = array_merge( array( 'blocksify' ), self::pro_plugin_slugs() );

		/**
		 * Filter the plugin slugs shown as Required in the wizard.
		 *
		 * @param string[] $slugs Required plugin directory slugs.
		 */
		$slugs = apply_filters( 'custstsi_required_plugin_slugs', $slugs );

		return array_values( array_unique( array_filter( array_map( 'sanitize_key', (array) $slugs ) ) ) );
	}

	/**
	 * Plugin slugs that mark a template as "Pro".
	 *
	 * A template is Pro when its `plugins[]` includes any of these. The
	 * list is filterable so the gated set can grow without a release.
	 *
	 * @return string[]
	 */
	public static function pro_plugin_slugs(): array {
		$slugs = array( 'customify-pro', 'blocksify-pro' );

		/**
		 * Filter the Pro plugin slugs used to gate templates.
		 *
		 * @param string[] $slugs Pro plugin directory slugs.
		 */
		$slugs = apply_filters( 'custstsi_pro_plugin_slugs', $slugs );

		return array_values( array_unique( array_filter( array_map( 'sanitize_key', (array) $slugs ) ) ) );
	}

	/**
	 * Which of the Pro plugin slugs are installed on this site.
	 *
	 * "Installed" includes inactive plugins — the importer activates a
	 * present-but-inactive Pro plugin during the run, so only a missing
	 * (never installed) Pro plugin blocks import.
	 *
	 * @return string[] Installed Pro slugs.
	 */
	public static function installed_pro_plugin_slugs(): array {
		...
		foreach ( array_keys( (array) get_plugins() ) as $plugin_file ) {
			// Plugin file is "dir/file.php"; the directory is the slug.
			$dir = strtok( $plugin_file, '/' );
			if ( is_string( $dir ) && '' !== $dir ) {
				$installed_dirs[ $dir ] = true;
			}
		}

		return array_values( array_filter(
			self::pro_plugin_slugs(),
			static function ( $slug ) use ( $installed_dirs ) {
				return isset( $installed_dirs[ $slug ] );
			}
		) );
	}

	/**
	 * Install/active state for every plugin on the site, keyed by slug.
	 *
	 * The wizard's Plugins step uses this to show which of a template's
	 * plugins are already present (and whether they're active), so it can
	 * skip installing them and label their status.
	 *
	 * @return array<string, array{installed: bool, active: bool}>
	 */
	public static function installed_plugin_states(): array {
		if ( ! function_exists( 'get_plugins' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}
		...
		$states = array();
		foreach ( (array) get_plugins() as $plugin_file => $unused ) {
			$slug = strtok( (string) $plugin_file, '/' );
			if ( ! is_string( $slug ) || '' === $slug ) {
				continue;
			}
			// If two plugin files share a directory slug, keep the active one.
			$active = is_plugin_active( $plugin_file );
			if ( ! isset( $states[ $slug ] ) ) {
				$states[ $slug ] = array( 'installed' => true, 'active' => $active );
			} elseif ( $active ) {
				$states[ $slug ]['active'] = true;
			}
		}
	 ...
	 *
	 *     add_filter( 'custstsi_curated_font_pairs', function ( $pairs ) {
	...
	private function curated_font_pairs(): array {
		 ...
		 */
		return (array) apply_filters( 'custstsi_curated_font_pairs', array() );
	}
	 ...
	 * @param \Customify_Starter_Sites\Jobs\Importer_Runner  $runner  Runner — used for logging.
	 */
	public function after_phase( string $phase, array $job, $runner ): void {
		// Installing Google Fonts into the WP Font Library is the single
		// slowest part of an import (network download + sideload + REST
		// insert for every weight variant — ~20s per family). It's
		// independent of the content/options layers, so we kick it off
		// early — right after the template assets are fetched — instead of
		// blocking the final "applying_options" phase on it. The install
		// runs while content is importing; the family ids it produces are
		// cached on this adapter instance (the runner keeps one instance
		// alive across a job's phases) and consumed later when we write the
		// typography theme_mods. Writing the mods must still happen in
		// `applying_options` so Options_Importer doesn't overwrite them.
		if ( 'fetching' === $phase ) {
			$this->preinstall_fonts( $job, $runner );
			return;
		}
	 ...
	 */
	/**
	 * Normalise the wizard's `style.font` payload into a font pair.
	 *
	 * @param mixed $font String id or {id, heading, body, weight} array.
	 * @return array|null { id, heading, body, weight } or null.
	 */
	private function resolve_font_pair( $font ): ?array {
		if ( is_array( $font ) ) {
		...
		if ( is_string( $font ) && '' !== $font ) {
			return $this->find_font_pair( $font );
		}
		return null;
	}

	/**
	 * Pre-install the template's fonts into the WP Font Library early
	 * (right after assets are fetched), so the slow network work overlaps
	 * with the content/options import instead of blocking the final phase.
	 * Caches the installed family ids on the instance for apply_typography().
	 *
	 * @param array  $job    Job record.
	 * @param object $runner Importer runner (for logging).
	 */
	private function preinstall_fonts( array $job, $runner ): void {
		$style = $job['config']['style'] ?? null;
		if ( ! is_array( $style ) ) {
			return;
		}
		$font = $style['font'] ?? null;
		$pair = $this->resolve_font_pair( $font );
		if ( null === $pair ) {
			return;
		}

		$job_id = (string) ( $job['id'] ?? '' );
		$store  = ( $runner && method_exists( $runner, 'job_store' ) ) ? $runner->job_store() : null;

		// Surface progress in the UI. Font install runs while status is
		// still `fetching`, but the wizard shows a dedicated "Installing
		// fonts" row keyed on the 8–15 percent band — bump the percent
		// into that band so the row lights up (and back to 15 when done).
		if ( $store && '' !== $job_id && method_exists( $store, 'set_progress' ) ) {
			$store->set_progress( $job_id, 8, 0, 0, __( 'Installing fonts…', 'customify-starter-sites' ) );
		}

		$installer = new Font_Installer();
		$heading_id = $installer->install( $pair['heading'] );
		$body_id    = $pair['heading'] === $pair['body']
			? $heading_id
			: $installer->install( $pair['body'] );

		if ( $store && '' !== $job_id && method_exists( $store, 'set_progress' ) ) {
			$store->set_progress( $job_id, 15 );
		}

		$this->preinstalled_fonts = array(
			'pair'       => $pair,
		...
		$this->log_runner( $runner, $job_id, sprintf(
			'Style: fonts pre-installed (heading=%s body=%s) — theme_mods applied after options.',
			$heading_id ? 'OK' : 'skip',
			$body_id ? 'OK' : 'skip'
		) );
	}
	...
	private function apply_typography( $font, $runner, string $job_id ): void {
		// Reuse the family ids from the early pre-install when the same
		// font pair was already handled in the `fetching` phase — avoids
		// re-downloading + re-installing every variant here (the slow bit).
		$pre  = $this->preinstalled_fonts;
		$pair = $this->resolve_font_pair( $font );
		if ( null === $pair ) {
			$this->log_runner( $runner, $job_id, 'Style: font pair not found, skipped.' );
			return;
		}

		if ( is_array( $pre ) && isset( $pre['pair']['id'] ) && $pre['pair']['id'] === $pair['id'] ) {
			$heading_installed = $pre['heading_id'];
			$body_installed    = $pre['body_id'];
		} else {
			// No matching pre-install (e.g. font changed, or fetching phase
			// didn't run it) — install now as a fallback.
			$installer = new Font_Installer();
	...
	public function redirect_standalone_to_embed(): void {
		...
		// Read-only navigation check on the admin `page` query var to
		// decide whether to redirect this pageview — not form processing,
		// so there is no nonce to verify (mirrors how WP routes admin menus).
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin routing check, no state change.
		if ( ! isset( $_GET['page'] ) ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin routing check, no state change.
		if ( Generic_Dashboard::PAGE_SLUG !== sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			return;
		}
