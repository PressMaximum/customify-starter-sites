 ...
 * theme. The default adapter ({@see Customify_Starter_Sites\Adapters\Default_Adapter})
 ...
 *
 * Adapter selection happens in {@see Customify_Starter_Sites\Theme_Detector}.
...
abstract class Theme_Adapter {
	 ...
	 * @param \Customify_Starter_Sites\Jobs\Importer_Runner $runner   Runner instance — adapter
	 ...
	 *
	 * Bootstrap registers a callback on `custstsi_default_studio_url`
	 ...
	 * `custstsi_studio_key` filter that consults this method.
	 *
	 * Return policy:
	...
	public function admin_label(): string {
		return __( 'Starter Templates', 'customify-starter-sites' );
	}
	 ...
	 * (Appearance submenu) and the `custstsi_show_generic_menu`
	...
	/**
	 * Where the "Import demo" link on the Plugins → customify-starter-sites
	 * row should send the user. Default: the Generic track's dashboard
	 * (`admin.php?page=customify-starter-sites`).
	 *
	...
	public function plugin_row_import_url(): ?string {
		return admin_url( 'admin.php?page=customify-starter-sites' );
	}
	...
	public function plugin_row_import_label(): string {
		return __( 'Import demo', 'customify-starter-sites' );
	}
	 ...
	 * call `window.customifyStarterSites.mount(el)` on its own.
	 */
	...
	/**
	 * Extra keys merged into `window.customifyStarterSites` for the React tree
