...
namespace Customify_Starter_Sites\Adapters\Customify;
...
class Font_Installer {
	...
	private function download_font_to_temp( string $url ): ?array {
		...
		if ( $current_ext !== $canonical_ext ) {
			...
			if ( $wp_filesystem && $wp_filesystem->move( $tmp_path, $new_path, true ) ) {
				$tmp_path = $new_path;
			}
		}
	...
	private function sideload_font_to_uploads( array $file ): ?string {
		...
		if ( ! is_array( $sideload ) || isset( $sideload['error'] ) || empty( $sideload['url'] ) ) {
			// Log (debug builds only) so we can see why uploads silently fail.
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				$err = is_array( $sideload ) && isset( $sideload['error'] ) ? (string) $sideload['error'] : 'unknown sideload error';
				error_log( '[custstsi font] sideload failed: ' . $err . ' (file=' . ( $file['name'] ?? '?' ) . ')' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Guarded by WP_DEBUG; diagnostic only.
			}
			return null;
		}
	...
	private function log_rest_error( string $kind, string $context, \WP_REST_Response $res ): void {
		...
		error_log( sprintf( '[custstsi font] REST %s install failed for %s: %s', $kind, $context, $msg ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Guarded by WP_DEBUG; diagnostic only.
	}
