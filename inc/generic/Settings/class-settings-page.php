...
class Settings_Page {
	...
	public const PAGE_SLUG    = 'customify-starter-sites';
	public const TAB_SETTINGS = 'settings';

	private Options_Store $options;
	/** @var \Customify_Starter_Sites\Studio\Remote_Client|null Provided after B3 lands. */
	private $client;
	...
	public function register(): void {
		...
		add_action( 'admin_post_custstsi_test', [ $this, 'handle_test_connection' ] );
	}

	public function register_settings(): void {
		register_setting( 'custstsi', Options_Store::OPT_STUDIO_URL, [
			'type'              => 'string',
			'sanitize_callback' => 'esc_url_raw',
			'default'           => '',
		] );
		register_setting( 'custstsi', Options_Store::OPT_STUDIO_KEY, [
			'type'              => 'string',
	...
	public function render(): void {
		...
		$result = get_transient( 'custstsi_test_result' );
		if ( $result ) {
			delete_transient( 'custstsi_test_result' );
		}
		?>
		<?php if ( $result ) : ?>
		...
		<form method="post" action="options.php" class="custstsi-settings-form">
			<?php settings_fields( 'custstsi' ); ?>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row">
						<label for="custstsi_studio_url"><?php esc_html_e( 'Studio server URL', 'customify-starter-sites' ); ?></label>
					</th>
					<td>
						<?php $url_locked = $this->options->is_studio_url_locked(); ?>
						<input
							type="url"
							id="custstsi_studio_url"
						...
						<p class="description">
							<?php if ( $url_locked ) : ?>
								...
								printf(
									...
									esc_html__( 'Locked by wp-config — the %s constant overrides this field.', 'customify-starter-sites' ),
									'<code>' . esc_html( Options_Store::CONST_STUDIO_URL ) . '</code>'
								);
								?>
							<?php else : ?>
								<?php esc_html_e( 'Base URL of the Blocksify Design Studio that supplies template demos. Trailing slash optional. Leave empty to use the plugin default.', 'customify-starter-sites' ); ?>
							<?php endif; ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="custstsi_studio_key"><?php esc_html_e( 'Read API key', 'customify-starter-sites' ); ?></label>
					</th>
					<td>
						<input
							type="password"
							id="custstsi_studio_key"
						...
						/>
						<p class="description">
							<?php esc_html_e( 'A `read`-scope key generated on the Studio. Stored in wp_options, never sent to the browser after save.', 'customify-starter-sites' ); ?>
		...
		<h2><?php esc_html_e( 'Test connection', 'customify-starter-sites' ); ?></h2>
		<p class="description">
			<?php esc_html_e( 'Sends a single GET /me request to the Studio with the saved key. Use this to verify the URL is reachable and the key is valid before running an import.', 'customify-starter-sites' ); ?>
		</p>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<?php wp_nonce_field( 'custstsi_test' ); ?>
			<input type="hidden" name="action" value="custstsi_test" />
			<p>
				<button type="submit" class="button button-secondary"<?php disabled( ! $this->options->has_credentials() ); ?>>
					<?php esc_html_e( 'Test connection to Studio', 'customify-starter-sites' ); ?>
				</button>
				<?php if ( ! $this->options->has_credentials() ) : ?>
					<span class="description"><?php esc_html_e( 'Save the URL + key first.', 'customify-starter-sites' ); ?></span>
				<?php endif; ?>
			</p>
		</form>
		<?php
	...
	public function handle_test_connection(): void {
		...
		check_admin_referer( 'custstsi_test' );

		if ( null === $this->client ) {
			$msg = __( 'Studio client not initialised — the generic background pipeline is still being wired (phase B3).', 'customify-starter-sites' );
			set_transient( 'custstsi_test_result', [ 'ok' => false, 'message' => $msg ], 60 );
		} else {
			$res = $this->client->get( 'me' );
			if ( ! empty( $res['error'] ) ) {
				$msg = sprintf(
					/* translators: %s: error message */
					__( 'Connection failed: %s', 'customify-starter-sites' ),
					$res['error']
				);
				set_transient( 'custstsi_test_result', [ 'ok' => false, 'message' => $msg ], 60 );
			} else {
				...
				$msg   = sprintf(
					...
					__( 'Connected. Key label: %1$s · scope: %2$s.', 'customify-starter-sites' ),
					$label,
					$scope
				);
				set_transient( 'custstsi_test_result', [ 'ok' => true, 'message' => $msg ], 60 );
			}
		}
