 ...
 * under `customify-starter-sites/v1/` and are gated by `manage_options` + the
...
class Studio_Proxy_Controller {
	 ...
	 * prefixed `custstsi_` (list / detail / options / categories), so
	...
	public function check_permission( \WP_REST_Request $request ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				'custstsi_forbidden',
				__( 'You need `manage_options` to use the importer.', 'customify-starter-sites' ),
				[ 'status' => 403 ]
			);
		}
	 ...
	 * same delete-on-bypass semantics, same `X-Custstsi-Cache` header
	 ...
	 *
	 * The cache key passed in is namespaced with `custstsi_`
	...
	private function cached_proxy( \WP_REST_Request $request, string $key, int $ttl_secs, callable $fetch ) {
		...
		if ( $no_cache ) {
			...
			if ( is_array( $cached ) && array_key_exists( 'body', $cached ) ) {
				...
				$response->header( 'X-Custstsi-Cache', 'HIT' );
				return $response;
			}
		}
		...
		$response->header( 'X-Custstsi-Cache', $no_cache ? 'BYPASS' : 'MISS' );
		return $response;
	}
	...
	public function list_templates( \WP_REST_Request $request ) {
		...
		return $this->cached_proxy( $request, $cache_key, self::LIST_TEMPLATES_TTL, function () use ( $query ) {
			...
			if ( 0 === $res['status'] && null !== $res['error'] ) {
				return new \WP_Error( 'custstsi_upstream_unreachable', $res['error'], [ 'status' => 502 ] );
			}
	...
	public function get_template( \WP_REST_Request $request ) {
		...
		if ( $id <= 0 ) {
			return new \WP_Error( 'custstsi_bad_template_id', 'Invalid template id.', [ 'status' => 400 ] );
		}
	...
	public function get_template_options( \WP_REST_Request $request ) {
		...
		if ( 0 === $detail_res['status'] && null !== $detail_res['error'] ) {
			return new \WP_Error( 'custstsi_upstream_unreachable', $detail_res['error'], [ 'status' => 502 ] );
		}
		$detail = $detail_res['body'];
		if ( ! is_array( $detail ) ) {
			return new \WP_Error( 'custstsi_bad_detail', 'Template detail not parseable.', [ 'status' => 502 ] );
		}
		$url = (string) ( $detail['assets']['options']['url'] ?? $detail['options_url'] ?? '' );
		if ( '' === $url ) {
			return new \WP_Error( 'custstsi_no_options_url', 'Template has no options.json URL.', [ 'status' => 404 ] );
		}
		...
		return $this->cached_proxy( $request, $cache_key, self::TEMPLATE_OPTIONS_TTL, function () use ( $url ) {
			...
			if ( is_wp_error( $res ) ) {
				return new \WP_Error( 'custstsi_options_unreachable', $res->get_error_message(), [ 'status' => 502 ] );
			}
			$code = (int) wp_remote_retrieve_response_code( $res );
			if ( $code < 200 || $code >= 300 ) {
				return new \WP_Error( 'custstsi_options_http_status', "options.json returned HTTP {$code}.", [ 'status' => 502 ] );
			}
			$body   = wp_remote_retrieve_body( $res );
			$parsed = json_decode( $body, true );
			if ( ! is_array( $parsed ) ) {
				return new \WP_Error( 'custstsi_options_not_json', 'options.json was not valid JSON.', [ 'status' => 502 ] );
			}
