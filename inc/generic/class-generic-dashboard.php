 ...
 * `.wrap.custstsi-app`, `.wp-heading-inline`, `.wp-filter` + tab strip,
 ...
 *     instead of `customify.com/wp-json/wp/v2/download/`.
 *   - Import runs as a background WP-Cron job (B4), not a synchronous
 *     `wp_ajax_custstsi__import` call.
 *   - The Underscore template `#tmpl-customify-starter-sites-preview` posts to
...
class Generic_Dashboard {
	...
	public function maybe_clear_cache(): void {
		...
		$adapter = apply_filters( 'custstsi_active_adapter', null );
	...
	public static function wipe_all_transients(): void {
		global $wpdb;
		$prefix         = $wpdb->esc_like( '_transient_custstsi_' ) . '%';
		$timeout_prefix = $wpdb->esc_like( '_transient_timeout_custstsi_' ) . '%';
		// Bulk-deleting this plugin's transient options in one prepared
		// statement; caching is irrelevant here (we are wiping the cache).
		$wpdb->query( $wpdb->prepare( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- One-off bulk transient cleanup via a prepared statement.
	...
	public function register_menu(): void {
		...
		// URL: `themes.php?page=customify-starter-sites` (and the catch-all
		// `admin.php?page=customify-starter-sites` also resolves).
		//
		// Adapter overrides:
		...
		//   - `custstsi_show_generic_menu` filter still wins when
		 ...
		 */
		$show_menu = (bool) apply_filters( 'custstsi_show_generic_menu', true, $adapter, $template, $stylesheet );
		...
		if ( ! $show_menu ) {
			 ...
			 * @param object $adapter  Active Theme_Adapter (or null).
			 */
			$parent = (string) apply_filters( 'custstsi_dashboard_parent_slug', $parent, $adapter );
		}
	...
	public function enqueue_assets( $hook ): void {
		...
		$build_dir = CUSTOMIFY_STARTER_SITES_PATH . 'inc/generic/build/';
		$build_url = CUSTOMIFY_STARTER_SITES_URL . 'inc/generic/build/';
		...
		// dashboard enqueues. We mirror its `.custstsi-app-themes-listing
		...
		wp_enqueue_style(
			...
			CUSTOMIFY_STARTER_SITES_URL . 'style.css',
			[],
			'1.2.0'
		);
		wp_enqueue_style(
			'customify-starter-sites-admin',
			$build_url . 'admin.css',
			[ 'wp-components', 'customify-starter-sites' ],
			$asset['version'] ?? '1.2.0'
		);

		$base = [
			'restRoot'          => esc_url_raw( rest_url( 'customify-starter-sites/v1' ) ),
			'restNonce'         => wp_create_nonce( 'wp_rest' ),
			'pollIntervalMs'    => 2000,
			'currentTheme'      => get_option( 'template' ),
			'currentStylesheet' => get_option( 'stylesheet' ),
			'adapterLabel'      => $adapter ? $adapter->admin_label() : __( 'Starter Templates', 'customify-starter-sites' ),
		...
		// `window.customifyStarterSites.mount(el)` on its own lifecycle.
		if ( $is_host && ! $own_page ) {
			$base['embedded'] = true;
		}
		...
		wp_localize_script( 'customify-starter-sites-admin', 'customifyStarterSites', $localize );
	}
	...
	public function dashboard(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'customify-starter-sites' ) );
		}

		?>
		<div class="wrap custstsi-app">
				...
				<div class="notice notice-error">
					<p>
						...
						printf(
							...
							esc_html__( 'React build output missing — run %s in the plugin directory.', 'customify-starter-sites' ),
	...
	// page <h1>, the tab strip, and the `<div id="customify-starter-sites-app">`
	// mount point.
}
