 ...
 * to `custstsi_*` so this plugin's state doesn't collide with
...
class Job_Store {
	...
	public const TRANSIENT_PREFIX = 'custstsi_job_';
	public const OPTION_LATEST    = 'custstsi_job_latest';
	/** Append-only list of template IDs that have ever completed an import.
	 *  Library UI reads this to render the "✓ Imported" badge. */
	public const OPTION_IMPORTED_TEMPLATES = 'custstsi_imported_templates';
	...
	public function create( array $config ): string {
		...
		$job = [
			...
			'progress'  => [
				...
				'current' => 0,
				'total'   => 0,
				'message' => __( 'Queued', 'customify-starter-sites' ),
			],
			'log'       => [],
			'warnings'  => [],
			'error'     => null,
	...
	public function complete( string $id, array $summary ): void {
		$this->update( $id, static function ( array $job ) use ( $summary ): array {
			...
			$job['progress']['message'] = __( 'Import complete.', 'customify-starter-sites' );
			$job['finished_at']         = current_time( 'mysql', true );
			return $job;
		} );
	 ...
	 *   1. Every pending `custstsi_run_job` cron event is
		...
		if ( null !== $job ) {
			...
			if ( ! in_array( $job['status'], $terminal, true ) ) {
				$this->update( $job['id'], static function ( array $j ): array {
					...
					$j['progress']['message'] = __( 'Cancelled — superseded by a new import.', 'customify-starter-sites' );
