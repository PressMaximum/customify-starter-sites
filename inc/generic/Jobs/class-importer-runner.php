 ...
 *     `custstsi_required_plugin_block` can downgrade.
...
use Customify_Starter_Sites\Steps\Asset_Fetcher;
use Customify_Starter_Sites\Steps\Content_Importer;
use Customify_Starter_Sites\Steps\Options_Importer;
use Customify_Starter_Sites\Steps\Plugin_Installer;
use Customify_Starter_Sites\Steps\Uploads_Extractor;

class Importer_Runner {

	public const CRON_HOOK = 'custstsi_run_job';
	...
	public function run( string $job_id ): void {
		...
		register_shutdown_function( function () use ( $job_id ) {
			...
			if ( is_array( $err ) && in_array( (int) $err['type'], [ E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR ], true ) ) {
				...
				$detail = __( 'the worker stopped unexpectedly (execution-time limit or fatal error)', 'customify-starter-sites' );
			}
			$this->jobs->fail( $job_id, sprintf(
				/* translators: 1: phase the import was in (e.g. installing_plugins), 2: fatal error detail */
				__( 'Import aborted during "%1$s": %2$s', 'customify-starter-sites' ),
				$status,
				$detail
			) );
		} );
		...
		try {
			...
			$this->jobs->set_status( $job_id, Job_Store::STATUS_FETCHING, __( 'Downloading template assets…', 'customify-starter-sites' ) );
			...
			$this->jobs->set_status( $job_id, Job_Store::STATUS_INSTALLING_PLUGINS, __( 'Installing required plugins…', 'customify-starter-sites' ) );
			...
			if ( ! empty( $config['import_uploads'] ) && '' !== $uploads_path ) {
				$this->jobs->set_status( $job_id, Job_Store::STATUS_EXTRACTING, __( 'Extracting media files…', 'customify-starter-sites' ) );
			...
			if ( ! empty( $config['import_content'] ) ) {
				$this->jobs->set_status( $job_id, Job_Store::STATUS_IMPORTING_CONTENT, __( 'Importing posts, terms, menus…', 'customify-starter-sites' ) );
			...
			if ( $this->cancelled( $job_id ) ) { $fetcher->cleanup( $job_id ); return; }
			$this->jobs->set_status( $job_id, Job_Store::STATUS_APPLYING_OPTIONS, __( 'Applying settings…', 'customify-starter-sites' ) );
	...
	private function cancelled( string $job_id ): bool {
		...
		$this->jobs->set_status( $job_id, Job_Store::STATUS_CANCELLED, __( 'Import cancelled by user.', 'customify-starter-sites' ) );
		return true;
	}
	...
	private function import_woo_attributes( string $options_path, string $job_id ): bool {
		...
		foreach ( $attrs as $a ) {
			...
			// $table is the WooCommerce attribute-taxonomies table name (a
			// table identifier can't be a prepared placeholder); the value
			// is passed through $wpdb->prepare().
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- $table is a trusted core table name; $name is prepared.
