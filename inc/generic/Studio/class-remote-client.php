...
namespace Customify_Starter_Sites\Studio;
...
class Remote_Client {
	...
	public function get( string $path, array $query = [] ): array {
		...
		if ( 'me' === $path ) {
			...
			return [
				...
				'body'   => null,
				'error'  => $ping['error'] ?? __( 'PM Templates catalog unreachable.', 'customify-starter-sites' ),
			];
		}
	...
	private function request( string $method, string $path, array $extra ): array {
		...
		if ( '' === $this->options->studio_url() ) {
			return [
				'status' => 0,
				'body'   => null,
				'error'  => __( 'Studio URL not configured.', 'customify-starter-sites' ),
			];
		}
	...
	private function dispatch_in_process( string $method, string $path, array $extra ): array {
		if ( ! did_action( 'rest_api_init' ) ) {
			rest_get_server();
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Firing WordPress core's own rest_api_init so an in-process REST dispatch has routes registered.
