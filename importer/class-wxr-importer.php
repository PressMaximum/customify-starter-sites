...
class Customify_Starter_Sites_WXR_Importer extends WP_Importer {
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
							$this->version,
	...
	public function import( $file ) {
		add_filter( 'custstsi_import_post_meta_key', array( $this, 'is_valid_meta_key' ) );
	...
	protected function log_error( WP_Error $error ) {
		...
		if ( ! empty( $data ) ) {
			$encoded = wp_json_encode( $data, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PARTIAL_OUTPUT_ON_ERROR );
			$this->logger->debug( false !== $encoded ? $encoded : '' );
		}
	}
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.',  'customify-starter-sites' ) );
		}
		 ...
		 */
		do_action( 'custstsi_import_start' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WordPress importer hook.
	}
	...
	protected function import_end() {
		 ...
		 */
		do_action( 'custstsi_import_end' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WordPress importer hook.
	}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping',  'customify-starter-sites' ) );
				$encoded = wp_json_encode( $map, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PARTIAL_OUTPUT_ON_ERROR );
				$this->logger->debug( false !== $encoded ? $encoded : '' );
				continue;
			}
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'customify-starter-sites' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		 ...
		 */
		$data = apply_filters( 'custstsi_wxr_importer.pre_process.post', $data, $meta, $comments, $terms ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__( '%s "%s" already exists.',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
			 ...
			 */
			do_action( 'custstsi_wxr_importer.process_already_imported.post', $data ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
		...
		$postdata = apply_filters( 'custstsi_wp_import_post_data_processed', $postdata, $data );

		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
					$data['post_title']
				) );
				/**
				 ...
				 */
				do_action( 'custstsi_wxr_importer.process_skipped.post', $data, $meta ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
				return false;
			}
            ...
            //$this->processed_posts[ $original_id ] = $post_id;
            // Fix error wrong maping media
			do_action( 'custstsi_wp_import_insert_post', $post_id, $original_id, $postdata, $data );
		}

		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%1s" (%2s)', 'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
			 ...
			 */
			do_action( 'custstsi_wxr_importer.process_failed.post', $post_id, $data, $meta, $comments, $terms ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			return false;
		}
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText
			$original_id,
			$post_id
		) );

		// Handle the terms too
		$terms = apply_filters( 'custstsi_wp_import_post_terms', $terms, $post_id, $data );
		...
		if ( ! empty( $terms ) ) {
			...
			foreach ( $term_ids as $tax => $ids ) {
				...
				do_action( 'custstsi_wp_import_set_post_terms', $tt_ids, $ids, $tax, $post_id, $data );
			}
		}
		 ...
		 * @param array $terms Raw term data, already processed.
		 */
		do_action( 'custstsi_wxr_importer.processed.post', $post_id, $data, $meta, $comments, $terms ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
	}
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			 ...
			 */
			$meta_item = apply_filters( 'custstsi_wxr_importer.pre_process.post_meta', $meta_item, $post_id ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			if ( empty( $meta_item ) ) {
				return false;
			}

			$key = apply_filters( 'custstsi_import_post_meta_key', $meta_item['key'], $post_id, $post ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WordPress importer hook.
			$value = false;
			...
			if ( $key ) {
				...
				do_action( 'custstsi_import_post_meta', $post_id, $key, $value ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WordPress importer hook.
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		$comments = apply_filters( 'custstsi_wp_import_post_comments', $comments, $post_id, $post );
		if ( empty( $comments ) ) {
			return 0;
		}
		...
		foreach ( $comments as $key => $comment ) {
			 ...
			 */
			$comment = apply_filters( 'custstsi_wxr_importer.pre_process.comment', $comment, $post_id ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			if ( empty( $comment ) ) {
				return false;
			}
			...
			if ( $post_exists ) {
				...
				if ( $existing ) {
					 ...
					 */
					do_action( 'custstsi_wxr_importer.process_already_imported.comment', $comment ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			 ...
			 * @param array $post Post data
			 */
			do_action( 'custstsi_wp_import_insert_comment', $comment_id, $comment, $post_id, $post );
			 ...
			 */
			do_action( 'custstsi_wxr_importer.processed.comment', $comment_id, $comment, $meta, $post_id ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
	...
	protected function process_author( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'custstsi_wxr_importer.pre_process.user', $data, $meta ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
				$userdata['user_login']
			) );
			 ...
			 */
			do_action( 'custstsi_wxr_importer.process_failed.user', $user_id, $userdata ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			return false;
		}
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %1d remapped to %2d',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
			$original_id,
			$user_id
		) );
		 ...
		 * @param array $userdata Raw data imported for the user.
		 */
		do_action( 'custstsi_wxr_importer.processed.user', $user_id, $userdata ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
	}
	...
	protected function process_term( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'custstsi_wxr_importer.pre_process.term', $data, $meta ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( $existing ) {
			 ...
			 */
			do_action( 'custstsi_wxr_importer.process_already_imported.term', $data ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
		...
		if ( is_wp_error( $result ) ) {
			...
			do_action( 'custstsi_wp_import_insert_term_failed', $result, $data );
			 ...
			 */
			do_action( 'custstsi_wxr_importer.process_failed.term', $result, $data, $meta ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
			return false;
		}
		...
		$this->logger->info( sprintf(
			__( 'Imported "%1s" (%2s)',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
			$data['name'],
			$data['taxonomy']
		) );
		$this->logger->debug( sprintf(
			__( 'Term %1d remapped to %2d',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
			$original_id,
			$term_id
		) );

		do_action( 'custstsi_wp_import_insert_term', $term_id, $data );
		 ...
		 * @param array $data Raw data imported for the term.
		 */
		do_action( 'custstsi_wxr_importer.processed.term', $term_id, $data ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WXR importer hook.
	}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			wp_delete_file( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size',  'customify-starter-sites' ) );
		}
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug( sprintf(
				...
				__( 'Running post-processing for post %d',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data',  'customify-starter-sites' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment, WordPress.WP.I18n.UnorderedPlaceholdersText	
					$comment_id
				) );
				continue;
			}
	...
	protected function max_attachment_size() {
		return apply_filters( 'custstsi_import_attachment_size_limit', 0 ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Backward-compatible WordPress importer hook.
	}
